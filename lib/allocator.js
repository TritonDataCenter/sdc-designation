...
var DEFAULT_DESC = [
    // keep hard-filter-invalid-servers and hard-filter-setup before any other
...
function (servers, vm, img, pkg, checkCapacity) {
    var self = this;
    ...
    var constraints = { vm: vm, img: img, pkg: pkg, capacity: checkCapacity };
    var result = self._dispatch(self.allocServerExpr, servers, constraints);
    var server = result[0][0];
    var visitedAlgorithms = result[1];
    var remainingServers = result[2];
    var reasonsRemoved = result[3];
...
Allocator.prototype.packageCapacity =
    ...
    packages.forEach(function (pkg) {
        images.forEach(function (img) {
            ...
            for (var i = 0; true; i++) {
                ...
                var server = self.allocate(simServers, vm, img, pkg, true)[0];
...
Allocator.prototype.serverCapacity =
function (servers) {
    var self = this;
    ...
    servers.forEach(function (server) {
        serversUnreserved[server.uuid] = {
            cpu:  server.unreserved_cpu,  // in cpu_cap's units
            ram:  server.unreserved_ram,  // in MiB
            disk: server.unreserved_disk  // in MiB
        };
    });
...
function (algorithms, servers, constraints) {
    ...
    return [ servers, visitedAlgorithms, remainingServers, reasonsRemoved ];
};
...
function (algorithms, initialServers, constraints) {
    ...
    for (var i = 0; i < algorithms.length; i++) {
        ...
        if (Array.isArray(algorithm)) {
            ...
            var result = algorithm.run(log, state, initialServers, constraints);
            servers = result[0];
            var reasons = result[1];
