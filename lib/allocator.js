...
var DEFAULT_DESC = [
    // probably a good idea to keep filter-setup before any other plugins to
    // avoid them possibly getting confused over missing values
...
function (servers, vmDetails, imgDetails, pkgDetails) {
    var self = this;

    // TODO: pkgDetails should be passed along to all plugins, not selectively
    // spliced into vmDetails
    if (pkgDetails) {
        vmDetails.traits = vmDetails.traits || pkgDetails.traits;

        vmDetails.overprovision_cpu  = +pkgDetails.overprovision_cpu;
        vmDetails.overprovision_ram  = +pkgDetails.overprovision_memory;
        vmDetails.overprovision_disk = +pkgDetails.overprovision_storage;
        vmDetails.overprovision_net  = +pkgDetails.overprovision_network;
        vmDetails.overprovision_io   = +pkgDetails.overprovision_io;
    }

    servers = self._massageServerData(servers);
    vmDetails = self._massageVmData(vmDetails);
...
function (servers, visitedAlgorithms, remainingServers) {
    ...
    steps.push({ 'Received by DAPI': initialServerUuids });

    for (var i = 0; i < visitedAlgorithms.length; i++) {
        var algoName = visitedAlgorithms[i].name;
        var step = {};
        step[algoName] = remainingServers[i];
        steps.push(step);
    }
...
/*
 * Computes what the unreserved CPU, RAM and disk on each server so that plugins
...
function (servers) {
    ...
    servers.forEach(function (server) {
        ...
        if (self.overprovision_ratios)
            var overprovisionRatios = self.overprovision_ratios[server.uuid];

        // a server that has no overprovision ratios is treated like a server
        // that never overprovisions RAM
        overprovisionRatios = overprovisionRatios ||
                              server.overprovision_ratios ||
                              DEFAULT_SERVER_OVERPROVISIONING;

        server.overprovision_ratios = overprovisionRatios;

        if (!vms)
            return;

        var overprovisionCpu  = overprovisionRatios.cpu;
        var overprovisionRam  = overprovisionRatios.ram;
        var overprovisionDisk = overprovisionRatios.disk;
...
/*
 * If a requested VM has no overprovision attributes, we treat it as if it had
 * an overprovision_ram of 1.0 (RAM won't overprovision, but other resources
...
Allocator.prototype._massageVmData =
function (vm) {
    if (!(vm.overprovision_cpu || vm.overprovision_ram ||
          vm.overprovision_disk || vm.overprovision_io ||
          vm.overprovision_net))
        vm.overprovision_ram = 1.0;
