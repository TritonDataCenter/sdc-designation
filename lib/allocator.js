...
function (servers, vmDetails, imgDetails, pkgDetails) {
    ...
    pkgDetails = self._massagePkgData(pkgDetails);
...
function (algorithms, servers, vmDetails, imgDetails, pkgDetails) {
    var self = this;
    var result;
...
function (algorithms, servers, vmDetails, imgDetails, pkgDetails) {
    ...
    for (var i = 0; i < algorithms.length && servers.length !== 0; i++) {
        ...
        servers = algorithm.run(log, state, servers, vmDetails, imgDetails,
                                pkgDetails);
...
function (algorithms, initialServers, vmDetails, imgDetails, pkgDetails) {
    var self = this;
    var log = self.log;
    var visitedAlgorithms = [];
    var remainingServers = [];
    ...
    for (var i = 0; i < algorithms.length; i++) {
        ...
        if (Array.isArray(algorithm)) {
            ...
            servers = algorithm.run(log, state, initialServers, vmDetails,
                                    imgDetails, pkgDetails);
            visitedAlgorithms.push(algorithm);
...
function (visitedAlgorithms, server, origServers, vmDetails, imgDetails,
    ...
    for (var i = visitedAlgorithms.length - 1; i >= 0; i--) {
        ...
        algorithm.post(log, state, server, origServers, vmDetails,
                       imgDetails, pkgDetails);
