...
function validVm(vm) {
    ...
    if (!vm.ram) {
        throw new restify.MissingParameterError('\'ram\' attribute not present' +
            ' in \'vm\' payload');
    }
...
/*
 * Validates if a server has enough space for the given vm
 */
function enoughSpace(server, vm) {
    var ramInBytes = vm.ram * 1024 * 1024;

    if (server['memory_available_bytes'] >= ramInBytes) {
        return true;
    } else {
        return false;
    }
...
function NotEnoughRamError(message) {
    restify.RestError.call(this, 409,
                           'NotEnoughRamError',
                           message,
                           NotEnoughRamError);

    this.name = 'NotEnoughRamError';
};

util.inherits(NotEnoughRamError, restify.RestError);
...
module.exports = {
    ...
    NotEnoughRamError: NotEnoughRamError
