...
function () {
    ...
    algorithmFiles.forEach(function (fileName) {
        ...
        if (algorithm) {
            algorithmLookup[fileName] = algorithm;
    ...
    var algorithmFlow = self.config.algorithms;
    if (!algorithmFlow) {
        algorithmFlow = DEFAULT_FLOW;
        self.log.info('No algorithm flow listed, using default');
    }

    var algorithms = self._loadFlow(algorithmFlow, algorithmLookup);
    self.algorithms = new Algorithms(self.log, algorithms);
};
...
function (req, res, next) {
    ...
    var vm = req.params.vm;
    var server = req.algorithms.allocate(req.servers,
                                         { ram:      vm.ram,
                                           disk:     vm.quota,
