...
function (req, res, next) {
    ...
    if (imgSize && typeof (imgSize) !== 'number')
        return next(new invalidErr('"image.image_size" has invalid type'));
...
function (req, res, next) {
    ...
    var parse = function (name, validator) {
        ...
        if (typeof (val) === 'string') {
            try {
                ...
                next(new invalidErr('"package.' + name + '" has invalid type'));
                return false;
            }
        }
    ...
    if (!parse('min_platform', validatePlatformLimits))
        return null;

    if (!parse('max_platform', validatePlatformLimits))
        return null;

    var traitShim = function () { return validateTraits(pkg.traits); };
    if (!parse('traits', traitShim))
        return null;
...
function (req, res, next) {
    ...
    var results = req.allocator.allocate(req.servers, vm, image, pkg);
