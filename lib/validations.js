    ...
    if (tags) {
        ...
        for (var i = 0; i !== tags.length; i++) {
            if (typeof (tags[i]) !== 'string')
                return 'VM "nic_tags" contains invalid type in array';
        }
    }
...
function validateServer(server) {
    ...
    if (typeof (nics) !== 'object' || Array.isArray(sysinfo)) {
        return 'Server ' + sUuid + ' "Network Interfaces" is not a hash';
    }

    var nicNames = Object.keys(nics);

    for (i = 0; i !== nicNames.length; i++) {
        var nic = nics[nicNames[i]];

        var nicStatus = nic['Link Status'];
        var nicTags   = nic['NIC Names'];

        if (typeof (nicStatus) !== 'string')
            return 'Server ' + sUuid + ' "Link Status" is not a string';

        if (!Array.isArray(nicTags))
            return 'Server ' + sUuid + ' "NIC Names" is not an array';

        for (var j = 0; j !== nicTags.length; j++) {
            var tag = nicTags[j];

            if (typeof (tag) !== 'string')
                return 'Server ' + sUuid + ' "NIC Names" contains non-string';
        }
    }
    ...
    if (server.vms) {
        ...
        for (i = 0; i != vmUuids.length; i++) {
            ...
            if (typeof (vm.last_modified) !== 'string')
                return 'VM ' + vmUuid + ' "last_modified" is not a string';

            if (typeof (vm.state) !== 'string')
                return 'VM ' + vmUuid + ' "state" is not a string';
        }
    }
