    ...
    servers.forEach(function (server) {
        ...
        for (var j = 0; j != vmUuids.length; j++) {
            ...
            if (cpu) {
                ...
                server.unreserved_cpu = 0;
                log.warn('Server', server.uuid, 'VM', vm.uuid,
                         'has no cpu_cap, assuming server is filled');
            }
...
function calcUnreservedDisk(server) {
    ...
    if (overprovisionDisk) {
        // We have to do some hackery here. Normally server.disk_kvm_quota_bytes
        // is enough (rather than the whole rigamole for kvmQuotaBytes), but
        // KVM quotas have been misapplied in production, so we work around that
        // for the time being.
        var kvmQuotaBytes = server.disk_kvm_zvol_volsize_bytes +
                            numKvms(server.vms) * 10 * GB;

        unreserved -= (server.disk_zone_quota_bytes + kvmQuotaBytes) /
                      overprovisionDisk;
    }
