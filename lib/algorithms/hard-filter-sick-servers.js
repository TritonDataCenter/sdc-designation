...
function filterSickServers(log, state, servers, constraints) {
...
function isSickServer(server) {
    var vms = server.vms;
    if (!vms)
        return null;

    var dayAgo = +new Date() - DAY_IN_MS;

    var newVms = Object.keys(vms).map(function (name) {
        return vms[name];
    }).filter(function (vm) {
