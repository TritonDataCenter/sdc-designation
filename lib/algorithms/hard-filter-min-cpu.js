...
function filterMinCpu(log, state, servers, constraints) {
    ...
    if (constraints.pkg.overprovision_cpu) {
        ...
        filter = function (server) {
            ...
            if (reasons) {
                var msg = 'VM\'s calculated ' + requestedCpu + ' CPU is ' +
                          'less than server\'s spare ' + server.unreserved_cpu;
        ...
        filter = function (server) {
            ...
            if (reasons) {
                var msg = 'Package gave no CPU overprovision ratio, but ' +
                          'server has ratio ' + serverCpu;
                reasons[server.uuid] = msg;
            }
