...
function filterInvalidServers(log, state, servers, constraints) {
    ...
    var validServers = servers.filter(function (server) {
        ...
        if (msg) {
            // This is an unfortunate workaround since amon triggers on this
            // warning, but unsetup servers return invalid server objects. We
            // don't want the amon probe firing on an unsetup server.
