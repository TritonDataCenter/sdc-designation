...
function filterMinRam(log, state, servers, constraints) {
    ...
    if (constraints.pkg.overprovision_ram) {
        ...
        filter = function (server) {
            ...
            if (reasons) {
                var msg = 'VM\'s calculated ' + requestedRam + ' RAM is ' +
                          'less than server\'s spare ' + server.unreserved_ram;
        ...
        filter = function (server) {
            ...
            if (reasons) {
                var msg = 'Package gave no RAM overprovision ratio, but ' +
                          'server has ratio ' + serverRam;
                reasons[server.uuid] = msg;
            }
