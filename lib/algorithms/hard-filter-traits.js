...
function filterTraits(log, state, servers, constraints) {
    ...
    var adequateServers = servers.filter(function (server) {
        ...
        if (!serverTraits ||
            ...
            if (reasons) {
                var msg = 'Combined vm/pkg/img traits require ' +
                          JSON.stringify(requestedTraits) + ' but server has ' +
        ...
        for (var i = 0; i !== traitNames.length; i++) {
            ...
            if (errMsg) {
                ...
                log.trace('Skipping server', server.UUID, 'because', errMsg);
...
function traitless(servers, reasons) {
    var traitlessServers = servers.filter(function (server) {
        ...
        if (reasons && !isTraitless) {
            reasons[server.uuid] = 'Combined vm/pkg/img require no traits ' +
                                   'but server has ' +
                                   JSON.stringify(server.traits);
        }
