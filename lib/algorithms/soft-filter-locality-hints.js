 ...
 * it can normalize.)
 *
 * "Strict" means that if the near/far requests cannot be satisfied, then
 * the allocation of a server will fail. Non-strict (i.e. `strict: false`)
 * means that if a `far` or `near` cannot be satisfied, then the rule is
 * dropped.
 *
 ...
 *
 * If no "locality" is given, this algoritm does a non-strict attempt to
 * filter out servers with VMs owned by this customer. In other words, by
 * default Triton attempts to spread out a customer's VMs. Customer-facing
 * documentation should promise no more than Triton making a reasonable
 * attempt to spread out customer VMs by default -- i.e. exactly how should
 * be left under-specified to allow for future changes. (Dev Note: Future
 * changes might be: better balancing, balancing by RBACv2 project,
 * balancing by image, limiting the number of CNs a customer can land on.
 * This balancing by default *overrides* subsequent sorting/scoring attempts
 * in the sdc-designation pipeline.)
 *
 *
 * # Limitations
 *
...
filterLocality(log, state, servers, constraints)
{
	...
	var far = normNearFar(locality.far, 'locality.far');
	var near = normNearFar(locality.near, 'locality.near');
	...
	if (!near && !far) {
		...
		if (far && far.length > 0) {
			servers = filterFar(servers, reasons, far, strict,
			    ownerUuid);
		}
		if (servers.length > 0 && near && near.length > 0) {
			servers = filterNear(servers, reasons, near, strict,
			    ownerUuid);
		}
	}
 ...
 * On the way out: undefined, or an array with 1 or more UUIDs.
 */
function normNearFar(nearFar, errStr) {
	if (typeof (nearFar) === 'string') {
		nearFar = [nearFar];
	}
	if (nearFar && nearFar.length === 0) {
		nearFar = undefined;
	}
	assert.optionalArrayOfUuid(nearFar, errStr);
	return (nearFar);
}
...
function softSpreadByOwner(servers, reasons, ownerUuid) {
	...
	for (i = 0; i < servers.length; i++) {
		...
		for (j = 0; j < serverVmUuids.length; j++) {
			if (server.vms[serverVmUuids[j]].owner_uuid
			    === ownerUuid) {
				exclude = true;
				break;
			}
...
function filterFar(servers, reasons, far, strict, ownerUuid) {
	...
	for (var i = 0; i < servers.length; i++) {
		...
		for (var j = 0; j < far.length; j++) {
			if (serverVms.hasOwnProperty(far[j]) &&
			    /* owner_uuid guard (see top comment) */
			    serverVms[far[j]].owner_uuid === ownerUuid)
			{
...
/*
 * Process `near`. Filter out any server not hosting all VMs listed
 * in `near`.
 *
 * `reasons` is modified in place.
 *
 * Dev Note: Seems to me the best way here is to get the server uuid
 * of the first VM in `near`. If that is available and hosts all
 * VMs in `near`, then use it. Else we filter out all servers.
 ...
 */
function filterNear(servers, reasons, near, strict, ownerUuid) {
	var filteredServers = [];
	var candidateServer;
	for (var i = 0; i < servers.length; i++) {
		var server = servers[i];
		if (server.vms.hasOwnProperty(near[0]) &&
		    /* owner_uuid guard (see top comment) */
		    server.vms[near[0]].owner_uuid === ownerUuid)
		{
