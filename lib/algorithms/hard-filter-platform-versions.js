...
function filterPlatformVersions(log, state, servers, constraints) {
    ...
    var pkgMinPlatforms = constraints.pkg.min_platform;
    if (pkgMinPlatforms)
...
function filterMinPlatforms(minPlatforms, servers, reasons) {
    ...
    for (var i = 0; i !== servers.length; i++) {
        ...
        if (version > maxVersion || platform >= minPlatform ||
            ...
            var msg = 'Image or package requires min platform "' + minPlatform +
                      '" for min version "' + minVersion + '", but server ' +
                      'has platform "' + platform + '" for version "' +
                      version + '"';
...
function filterMaxPlatforms(maxPlatforms, servers, reasons) {
    ...
    for (var i = 0; i !== servers.length; i++) {
        ...
        if (version < minVersion || platform <= maxPlatform ||
            ...
            var msg = 'Image or package requires max platform "' + maxPlatform +
                      '" for max version "' + maxVersion + '", but server ' +
                      'has platform "' + platform + '" for version "' +
                      version + '"';
            reasons[server.uuid] = msg;
        }
