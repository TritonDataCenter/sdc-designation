...
function addRecentVms(log, state, servers) {
    var recentVms = updateRecentVms(state);
    servers = addVmsToServers(servers, recentVms);
...
function addVmsToServers(servers, recentVms) {
    var serverLookup = {};

    Object.keys(recentVms).forEach(function (vmUuid) {
        var vm = recentVms[vmUuid];
    ...
    for (var i = 0; i !== servers.length; i++) {
        ...
        addVms.forEach(function (vm) {
            ...
            if (vm.image_size)
                server.disk_kvm_zvol_volsize_bytes += vm.image_size;

            if (vm.brand === 'kvm') {
                server.disk_kvm_zvol_volsize_bytes += vm.quota * GiB;
            } else {
...
function addNewVm(log, state, server, servers, constraints) {
    var vmDetails = constraints.vm;
    var pkg = constraints.pkg;
    var img = constraints.img;

    // this is here to aid testing
    if (vmDetails.override_recent_vms)
        return;
