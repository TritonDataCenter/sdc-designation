...
function filterTickets(log, state, servers, constraints) {
    ...
    var unprovisioningServers = servers.filter(function (server) {
        ...
        if (vmUuid) {
            if (reasons) {
                var msg = 'Server is currently provisioning VM ' + vmUuid;
...
function findOpenTickets(tickets) {
    var serverTickets = {};

    tickets.forEach(function (ticket) {
        if (ticket.scope === 'vm' && ticket.action === 'provision' &&
            (ticket.status === 'queued' || ticket.status === 'active')) {

          var serverUuid = ticket.server_uuid;
          serverTickets[serverUuid] = serverTickets[serverUuid] || [];
...
function getProvisioningVm(server, serversWithOpenTickets) {
    ...
    for (var i = 0; i !== openTickets.length; i++) {
        ...
        // we're looking for provisioning vms which the server does not know
        // about yet
        if (!vms[vmUuid])
            return vmUuid;
    }
