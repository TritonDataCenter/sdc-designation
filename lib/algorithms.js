...
var RESERVED_ARC_RATIO = 0.25;  // how much RAM on each server is left for ARC
...
function allocate(log, algorithms, servers, ram, disk, vlans) {
    var i = 0;
    ...
    for (; i < algorithms.length && servers.length !== 0; i++) {
        ...
        servers = algorithm.run(log, localState, servers, ram, disk, vlans);
...
function massageServerData(servers) {
    for (var i = 0; i != servers.length; i++) {
        ...
        server.unreserved_disk = server['Zpool Size in GiB'];
        server.unreserved_ram  = (server.memory_total_bytes / 1024 / 1024) *
                                 (1 - RESERVED_ARC_RATIO);
